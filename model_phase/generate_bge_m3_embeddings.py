"""
BGE-M3 Embedding Generator with Checkpoint Support

This script generates embeddings using BGE-M3 model and saves them for downstream classifiers.
...
class BGEM3EmbeddingGenerator:
    """Generate BGE-M3 embeddings with checkpoint support."""
    ...
    def generate_all_embeddings(self, train_data, val_data, test_data, use_wandb=False):
        ...
        print("\n" + "="*60)
        print("Generating BGE-M3 Embeddings with Checkpoint Support")
        print("="*60)
...
def main(dataset_name,
    ...
    print("\n" + "="*60)
    print("BGE-M3 Embedding Generation with Checkpoint Support")
    print("="*60)
    ...
    if output_dir is None:
        if experiment_name:
            ...
            output_dir = f'model_phase/results/bge_m3_embeddings_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    ...
    if use_wandb and WANDB_AVAILABLE:
        try:
            ...
            run_name = experiment_name or f"bge_m3_embeddings_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"\n{'='*60}")
    print("Initializing BGE-M3 Embedding Generator")
    print(f"{'='*60}")
    
    generator = BGEM3EmbeddingGenerator(
        max_length=max_length,
        batch_size=batch_size,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Generate BGE-M3 embeddings with checkpoint support')
